...
void fim (COBRA *cobra, MONITOR *monitor)
{
    ...
    setPosicaoJanela(monitor->pocisao_inicial.X, monitor->pocisao_inicial.Y);
    setDimensaoJanela(monitor->tamanho_inicial.X, monitor->tamanho_inicial.Y);
    ...
    printf("score = %d", cobra->score);
    gotoxy(meiox - 14, meioy - 1);
    printf("nome do jogador: %s", cobra->nome);
